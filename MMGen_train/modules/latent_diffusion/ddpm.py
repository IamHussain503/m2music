...
from MMGen_train.conditional_models import *
import datetime
import json

from MMGen_train.utilities.model_util import (
    exists,
    default,
    mean_flat,
    count_params,
...
from MMGen_train.utilities.diffusion_util import (
    make_beta_schedule,
    extract_into_tensor,
    noise_like,
)

from MMGen_train.modules.diffusionmodules.ema import LitEma
from MMGen_train.modules.diffusionmodules.distributions import (
    normal_kl,
    DiagonalGaussianDistribution,
)


from MMGen_train.modules.latent_diffusion.ddim import DDIMSampler
from MMGen_train.modules.latent_diffusion.plms import PLMSSampler
import soundfile as sf
import os
import sys
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from faiss_test import FaissDatasetBuilder
...
#            pretrained_path="/root/AudioLDM-training-finetuning/data/checkpoints/clap_music_speech_audioset_epoch_15_esc_89.98.pt",
            pretrained_path="/root/backup_m2music/checkpoints/clmp_epoch_30.pth",
...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cond_dict_why_history = []
        self.embeddings_collection = []
        self.learned_conditioning_history = []
        self.file_name_to_index = {}  
    ...
    def get_input(
        ...
        melody_npy = np.load("/root/backup_m2music/clm_data/embeddings/melody_embeddings.npy")
        melody_builder = FaissDatasetBuilder(melody_npy)
        melody_builder.load_index("/root/backup_m2music/clm_data/faiss/audio_2_melody_hnsw.faiss")
        # change the melody_npy and melody.faiss to the local path
...
class DiffusionWrapper(pl.LightningModule):
    ...
    #     print("Epoch ended, saving y values...")
    #     if self.all_y_values:
    #         all_y_values_concat = np.concatenate(self.all_y_values, axis=0)
    
    #         save_file_path = os.path.join("/work/home/acdfqcivpi/AudioLDM-training-finetuning-main_all_run/", f'y_values_epoch_{epoch_num}.npy')
    #         np.save(save_file_path, all_y_values_concat)
...
class LatentDiffusionVAELearnable(LatentDiffusion):
    ...
    def shared_step(self, batch, **kwargs):
        ...
        print("shared_step c type", type(c))
        print("shared_step c keys", c.keys())
